function ATeachIRB(robot, varargin)
    %-------------------------------
   ...
   %% install the panel at the side of the figure (Teach Panel)
	...
	if isempty(c)
		...
		if isempty(c)
			...
			robot.model.plot(zeros(1, robot.model.n));% get first axis holding the robot
			ax = gca;
        else
...
%% ---- set up the position display box
    ...
    for j = 1:n
        ...
        set(handles.edit(j), ...
            'Interruptible', 'off', ...
            'Callback', @(src,event)ATeachIRB_callback(src, robot.model.name, j, handles));
...
%% ATeachIRB Callback
function ATeachIRB_callback(src, name, j, handles)
	...
	if j < 4 
		...
		qMatrix = handles.robot.PathIK(Qang0, Posq1, 5); %Number of steps for position change
